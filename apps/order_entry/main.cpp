...
int main(int argc, char* argv[]) {
	...
	const auto all_trades = csv::tryParse<TradeMessage>(inf);
	if (!all_trades) {
		std::cerr << "Error parsing " << filename << ": " << describe(all_trades.error()) << std::endl;
		return 1;
	}
	...
	for (const auto& trade : *all_trades) {
		if (!symbol.empty() && symbol_view(trade.symbol) != symbol) continue;

		const std::uint64_t ts = trade.ts_ns;
		...
		const auto written = encode(trade, frame);
		if (!written) {
